...
:zap: **EDA 전체결론**
- Target변수(비혼잡함, 혼잡함)가 약 9대1로 불균형한 분포를 나타납니다.
- 소수의 지하철역에서 하차인원이 집중 된다는 점을 확인했습니다.
- 평일과 주말간의 뚜렷한 차이가 나타납니다. 평일에 주말보다 높은 하차인원을 보이는데 이를 통해 출퇴근 유동인구의 영향이 크다는 점을 알 수 있습니다.
- 하차인원은 날씨변수(기온, 풍속 등)보다 사업체수, 상권수, 인구수 와 같은 지역적 특성변수에 더 큰 영향을 받는다는 점을 알 수 있었습니다.

<br>

:zap: **EDA&모델링 방향성 확인**
- 모델링에 앞서 EDA를 통해 각 Feature간의 단위의 차이를 확인하였습니다. 이 차이를 해결하기 위해 StandardScaler를 사용하고, 각 분류기의 성능을 비교분석하기 위해 6가지의 모델을 설정한 Pipeline 구축하고자 합니다.
- Target변수의 불균형성을 확인했습니다. 이는 모델링에 악영향을 미칠 수 있으므로 Upsampling을 통해 해소 하고자 했습니다.
- EDA를 통해서 Feature간의 구분되는 특징들을 확인 할 수 있었습니다. 이를 모델링에 반영하고자 5가지의 Case별로 Feature들을 나누어 모델링을 진행하려고 합니다.
