...
- 코로나로 인한  데이터 부족
  - ~2019년 : 코로나 이전 / 2020년 ~ : 코로나 이후 
  - 코로나 이전, 이후의 데이터양상이 매우 상이함  
  - 5년치 데이터 사용시, **코로나 변화된 상황이 반영되지 않음**
- 사업체수 등 변화 반영 미흡
  - 코로나로 인한 정보 업데이트 지연   
...
- 혼잡 여부 산출식의 주관성
  <br>  <혼잡지표 관련 연구>  
  - 지하철 내부 혼잡도 측정 연구 
  - 통신데이터 기반 유동인구데이터를 활용한 혼잡 지표 개발 연구 <br>
✔ 그러나, **하차인원**을 활용한 지하철 역 인근의 혼잡을 판단하는 연구는 아직 존재하지 않음 
- 혼잡 여부를 판단하는 추가적인 데이터 확보 필요성 잔재
  - 혼잡을 표현할 수 있는 다른 데이터가 있다면 라벨로서 더 적합할 수 있음 

<br>

:bulb: **데이터 불균형**
- 데이터 통일성 부족
  - 데이터 전처리를 위한 추가적인 비용 필요  
- 결측치 처리에 따른 성능 저하 
  - 특정 지하철 역의 하차인원 데이터 누락 
- 특정 지하철역의 극단적인 이상값의 쏠림
  - ex) 혼잡비율의 outlier를 모두 제거시 신림역의 데이터 절반 이상이 제거가 됨 

<br>

:bulb: **Accuracy 대비 낮은 F1 Score**
- 적은 타겟의 수 
  - 산출식에 의해 도출된 혼잡(1) 라벨이 매우 적음
- F1 Score를 높일 수 있는 추가적인 데이터 수집 고려  
  -  ex) 행정동의 면적데이터, 지하철 역의 면적 데이터 등   
---
